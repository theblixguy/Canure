...
Imports WebsitesScreenshot.WebsitesScreenshot
...
Public Class MetroForm
    ...
    Dim mg_enable As Boolean
    Dim mg_x, mg_y, mg_dist As Integer
    Dim mg_direction As String
    Dim fcio As Bitmap
    ...
    Dim NotifyNetworkAvail As Boolean
    Dim EndVal As Integer '// 1 = NewTabPage() on Form_Shown() 2 = Default
...
#End Region
    ...
    Private Sub MetroForm_Invalidated(ByVal sender As Object, ByVal e As System.Windows.Forms.InvalidateEventArgs) Handles Me.Invalidated
    ...
    Private Sub MetroForm_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        meproc.PriorityClass = ProcessPriorityClass.RealTime
        ...
        Else
            Try
                If Not Command() = Nothing Then
                    ...
                    EndVal = 2
                ElseIf Not startup_page = Nothing Then
                    Me.Visible = True
...
#End Region
    ...
    Private Sub WebKitBrowser1_DownloadBegin(ByVal sender As Object, ByVal e As WebKit.FileDownloadBeginEventArgs) Handles WebKitBrowser1.DownloadBegin
        My.Computer.FileSystem.WriteAllText(Application.StartupPath & "/" & "applog.txt", Environment.NewLine & "[" & Date.Now & "] -> Download event fired.", True)
    End Sub
    ...
    Private Sub PictureBox3_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles PictureBox3.Click
        ...
        If RadDropDownList1.Text = "canure:home" Then
            WebKitBrowser1.Navigate(startup_page)
    ...
    Private Sub WebKitBrowser1_GeolocationPositionRequest(ByVal sender As Object, ByVal e As WebKit.GeolocationRequestEventArgs) Handles WebKitBrowser1.GeolocationPositionRequest
        e.Allow = True
    End Sub
    ...
    Private Sub WebKitBrowser1_Navigating(ByVal sender As Object, ByVal e As WebKit.WebKitBrowserNavigatingEventArgs) Handles WebKitBrowser1.Navigating

        Try
            Me.Controls.Remove(Ntp)
        Catch ex As Exception : End Try
    ...
    Private Sub WebKitBrowser1_NewWindowRequest(ByVal sender As Object, ByVal e As WebKit.NewWindowRequestEventArgs) Handles WebKitBrowser1.NewWindowRequest
        My.Computer.FileSystem.WriteAllText(Application.StartupPath & "/" & "applog.txt", Environment.NewLine & "[" & Date.Now & "] -> NewWindowRequest event fired. Browser doesnt handle these requests (currently) due to bug in engine.", True)
    End Sub
    ...
    Private Sub MetroForm_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown
        ...
        If My.Settings.homepage = "canure:welcome" Or My.Settings.homepage = "" Or EndVal = 1 Then
            Me.Visible = True
            Me.ShowInTaskbar = True
            Ntp.Location = Me.WebKitBrowser1.Location
            Ntp.Size = Me.WebKitBrowser1.Size
            Me.Controls.Add(Ntp)
            Ntp.Visible = True
            Ntp.BringToFront()
        End If
    ...
    Private Sub BackgroundWorker2_DoWork(ByVal sender As System.Object, ByVal e As System.ComponentModel.DoWorkEventArgs) Handles BackgroundWorker2.DoWork
        ...
        If My.Settings.datasend = True Then
            ...
            If My.Settings.browserGUID = Nothing Then
                ...
                My.Computer.FileSystem.WriteAllText(name & ".ceip", "OS NAME: " & osName & " | Build ID: " & BuildID & " | [Canure 2.3 Release Candidate]", False)
    ...
    Private Sub WebKitBrowser1_PluginFailed(ByVal sender As Object, ByVal e As WebKit.PluginFailedErrorEventArgs) Handles WebKitBrowser1.PluginFailed
        MsgBox("A plugin failed to load properly. More information: " & e.ErrorDescription, MsgBoxStyle.Critical, "Plugin Initialization Failed")
    End Sub


    Private Sub WebKitBrowser1_ProgressChanged1(ByVal sender As Object, ByVal e As WebKit.ProgressChangesEventArgs) Handles WebKitBrowser1.ProgressChanged
        Try

            RadProgressBar1.Visible = True
            RadProgressBar1.BringToFront()
            tbp.Instance.SetProgressState(TaskbarProgressBarState.Normal, Me.Handle)
            tbp.Instance.SetProgressValue(e.Percent, 100)
            RadProgressBar1.Value1 = e.Percent

        Catch ex As Exception : End Try
    End Sub

    Private Sub WebKitBrowser1_ShowJavaScriptAlertPanel(ByVal sender As Object, ByVal e As WebKit.ShowJavaScriptAlertPanelEventArgs) Handles WebKitBrowser1.ShowJavaScriptAlertPanel
        MsgBox(e.Message)
    End Sub

    Public Sub AddNewTabPage()
        Dim Ntp As New NewTabPage
        Ntp.Location = Me.WebKitBrowser1.Location
        Ntp.Size = Me.WebKitBrowser1.Size
        Me.Controls.Add(Ntp)
    ...
    Public Sub LoadPrefs()
        WebKitBrowser1.EnableHTTPPipelining = My.Settings.httppipeline
        WebKitBrowser1.WebView.preferences().setAcceleratedCompositingEnabled(Convert.ToInt32(True))
        WebKitBrowser1.WebView.preferences().setPlugInsEnabled(Convert.ToInt32(My.Settings.useplugins))
        WebKitBrowser1.WebView.preferences().setJavaEnabled(Convert.ToInt32(My.Settings.useJava))
        WebKitBrowser1.WebView.preferences().setJavaScriptEnabled(Convert.ToInt32(My.Settings.useJS))
        Timer4.Enabled = My.Settings.NotifyNetworkChange
        NotifyNetworkAvail = My.Settings.NotifyNetworkChange
        Timer4.Interval = My.Settings.NotifyNetworkChangeInterval
        WebKitBrowser1.Preferences.WillHandleDownloadsManually = True
        Dim DownFolder As String = "\" & "Downloads\"
        Dim MyDocs As String = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments)
        WebKitBrowser1.Preferences.DownloadsFolder = MyDocs + DownFolder
    End Sub

    Private Sub Timer4_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Timer4.Tick
        If NotifyNetworkAvail = True Then
            If System.Net.NetworkInformation.NetworkInterface.GetIsNetworkAvailable = False Then
                Panel13.BackColor = Panel2.BackColor
    ...
    Private Sub RadButton4_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles RadButton4.Click
        NotifyNetworkAvail = False
        Timer4.Enabled = False
        Panel13.Visible = False
    End Sub

    Public Sub ApplyOverallTheme()
        If My.Settings.Theme = "Blue" Then
            ThemeResolutionService.LoadPackageFile(Application.StartupPath & "/" & "Themes" & "/" & "TelerikMetroBlue.tssp")
            ThemeResolutionService.ApplicationThemeName = "TelerikMetroBlue"
        ElseIf My.Settings.Theme = "Green" Then
            ThemeResolutionService.LoadPackageFile(Application.StartupPath & "/" & "Themes" & "/" & "TelerikMetro.tssp")
            ThemeResolutionService.ApplicationThemeName = "TelerikMetro"
        End If
    End Sub
    '// BUGGY CODE BELOW ----->
    '
    'Public Sub CheckUIFontExists()
    'Dim SWP As Boolean
    'Dim SWPSLR As Boolean
    'SWP = IsFontInstalled("Segoe WP")
    'If SWP = True Then
    ' My.Computer.FileSystem.WriteAllText(Application.StartupPath & "/" & "applog.txt", Environment.NewLine & "[" & Date.Now & "] -> Fonts are properly installed. Checking passed", True)
    'Else
    'MsgBox("User Interface fonts are either missing or damaged. Canure will now try to install fonts. Installation will take a few seconds and after installation is complete, start Canure again", MsgBoxStyle.Exclamation, "Fonts Missing")
    'InstallUIFonts()
    'End If
    'End Sub
    ...
    'End Using
    ' Catch ex As Exception
    ' My.Computer.FileSystem.WriteAllText(Application.StartupPath & "/" & "applog.txt", Environment.NewLine & "[" & Date.Now & "] -> Error checking for fonts [" & ex.Data.ToString & "]", True)
    ' MsgBox("Font checking failed. Refer to the application log for more information. Canure will now exit [Please run 'fontinstaller.exe' present in Canure's folder to fix this issue]", MsgBoxStyle.Critical, "Font Check Failed")
    ' Application.Exit()
    'End Try
    'End Function
